...
import com.ark.robokart_robotics.Adapters.PlayCheckAnswerAdapter;
...
public class PlayQuizActivity extends AppCompatActivity {
    ...
    private com.ark.robokart_robotics.Activities.Quiz.PlayQuizViewModel quizViewModel;
...
right_answer=0;
    ...
    public void init() {
        ...
        checkAnswerAdapter = new PlayCheckAnswerAdapter(getApplicationContext(),questionArrayList);





        quizViewModel = new ViewModelProvider(this).get(PlayQuizViewModel.class);
...
//                startTimer();
                ...
                startActivity(new Intent(getApplicationContext(), com.ark.robokart_robotics.Activities.Quiz.PlayQuizActivity.class));
                finish();
        ...
        go_home_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                finish();
                startActivity(new Intent(PlayQuizActivity.this,DailyQuizActivity.class));
...
// Not calling **super**, disables back button in current screen.
    ...
    public MutableLiveData<List<Question>> getQuestionList() {
        ...
        StringRequest request = new StringRequest(Request.Method.POST, ApiConstants.HOST + ApiConstants.daily_quiz_api, response -> {
            try {
...
//                        Toast.makeText(getActivity(), e.getMessage(), Toast.LENGTH_SHORT).show();
    ...
    private void showNextQuestion(List<Question> questionList) {
        rbGroup.clearCheck();
        //Log.i("question counter",""+questionCounter+" counter:"+counter);
       // for (int v=0;v!=questionCountTotal;v++) {
            //if (counter != 0 && answersGivenList.size() > counter+1)
        ...
        if (questionCounter < questionCountTotal) {
            ...
            tvCorrectAnswers.setText(String.valueOf(right_answer));
            ...
            wrong = ""+(Integer.parseInt(count)-Integer.parseInt(right));
            ...
            if(passing < 60) {
                ...
                submitQuizResult(quiz_id,customer_id,username,right,wrong,count,percent);
            }
            else {
        ...
        Log.i("NextClickPLayQuiz","ques count:"+questionCounter+"&ansgivensize:"+answersGivenList.size());
    ...
    private void showPreviousQuestion(List<Question> questionList) {
        ...
        Log.i("Prev Click PLayQuiz","ques count:"+questionCounter+"&ansgivensize:"+answersGivenList.size());
        if (questionCounter<=answersGivenList.size())
            rbGroup.check(Integer.parseInt(answersGivenList.get(questionCounter-1)));
    ...
    //On Last Question
    public void submitQuizResult(String quiz_id, String customer_id,
                                 String user, String total_right,
                                 String total_wrong, String total, String percent){
        StringRequest request = new StringRequest(Request.Method.POST, ApiConstants.HOST + ApiConstants.submit_daily_quiz, response -> {
            try {
                JSONObject jsonObject = new JSONObject(response);
                int status = jsonObject.getInt("statusId");
            Log.i("QuizResult Respo",response);
                if (status == 1) {
